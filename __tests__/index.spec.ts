...
describe('urlLiteral', () => {
  ...
  describe('query method', () => {
    ...
    test('should support chaining with params', () => {
      const url = urlLiteral`/api/users/:id`.param({ id: 123 }).query({ page: 1, limit: 10 });
      expect(url.toString()).toBe('/api/users/123?page=1&limit=10');
    });

    test('should support chaining query then params', () => {
      const url = urlLiteral`/api/users/:id`.query({ page: 1 }).param({ id: 123 });
  ...
  describe('params method', () => {
    test('should replace single path parameter', () => {
      const url = urlLiteral`/api/users/:id`.param({ id: 123 });
      expect(url.toString()).toBe('/api/users/123');
    });

    test('should replace multiple path parameters', () => {
      const url = urlLiteral`/api/users/:userId/posts/:postId`.param({ userId: 123, postId: 456 });
      expect(url.toString()).toBe('/api/users/123/posts/456');
    });

    test('should handle empty object and return original path', () => {
      const url = urlLiteral`/api/users/:id`.param({});
      expect(url.toString()).toBe('/api/users/:id');
    });

    test('should handle null parameter and return original path', () => {
      const url = urlLiteral`/api/users/:id`.param(null);
      expect(url.toString()).toBe('/api/users/:id');
    });

    test('should handle undefined parameter and return original path', () => {
      const url = urlLiteral`/api/users/:id`.param(undefined);
      expect(url.toString()).toBe('/api/users/:id');
    });

    test('should ignore null/undefined values in params', () => {
      const url = urlLiteral`/api/users/:id/posts/:postId`.param({ id: 123, postId: null });
      expect(url.toString()).toBe('/api/users/123/posts/:postId');
    });

    test('should replace path parameters with string values', () => {
      const url = urlLiteral`/api/categories/:category`.param({ category: 'tech' });
      expect(url.toString()).toBe('/api/categories/tech');
    });

    test('should replace path parameters with numeric values', () => {
      const url = urlLiteral`/api/users/:id`.param({ id: 0 });
    ...
    test('should handle path parameters in the middle of path', () => {
      const url = urlLiteral`/api/users/:id/posts`.param({ id: 123 });
  ...
  describe('toString method', () => {
    ...
    test('toString should reflect params after calling params method', () => {
      const urlObj = urlLiteral`/api/users/:id`.param({ id: 123 });
    ...
    test('toString should include both params and query when chained', () => {
      const url = urlLiteral`/api/users/:id`.param({ id: 123 }).query({ page: 1 });
      expect(url.toString()).toBe('/api/users/123?page=1');
    });
  });
  ...
  describe('paramsString method', () => {
    test('should replace single path parameter and return string directly', () => {
      const result = urlLiteral`/api/users/:id`.paramString({ id: 123 });
      expect(result).toBe('/api/users/123');
    });

    test('should replace multiple path parameters and return string directly', () => {
      const result = urlLiteral`/api/users/:userId/posts/:postId`.paramString({ userId: 123, postId: 456 });
      expect(result).toBe('/api/users/123/posts/456');
    });

    test('should handle empty object and return original path', () => {
      const result = urlLiteral`/api/users/:id`.paramString({});
      expect(result).toBe('/api/users/:id');
    });

    test('should handle null parameter and return original path', () => {
      const result = urlLiteral`/api/users/:id`.paramString(null);
      expect(result).toBe('/api/users/:id');
    });

    test('should handle undefined parameter and return original path', () => {
      const result = urlLiteral`/api/users/:id`.paramString(undefined);
      expect(result).toBe('/api/users/:id');
    });

    test('should ignore null/undefined values in params', () => {
      const result = urlLiteral`/api/users/:id/posts/:postId`.paramString({ id: 123, postId: null });
      expect(result).toBe('/api/users/123/posts/:postId');
    });

    test('should replace path parameters with string values', () => {
      const result = urlLiteral`/api/categories/:category`.paramString({ category: 'tech' });
      expect(result).toBe('/api/categories/tech');
    });

    test('should replace path parameters with numeric values', () => {
      const result = urlLiteral`/api/users/:id`.paramString({ id: 0 });
    ...
    test('should handle path parameters in the middle of path', () => {
      const result = urlLiteral`/api/users/:id/posts`.paramString({ id: 123 });
    ...
    test('should work with combined path variables and params', () => {
      const userId = 456;
      const result = urlLiteral`/api/users/${userId}/posts/:postId`.paramString({ postId: 789 });
  ...
  describe('queryString method', () => {
    test('should add a single query parameter and return string directly', () => {
      const result = urlLiteral`/api/users`.queryString({ page: 1 });
      expect(result).toBe('/api/users?page=1');
    });

    test('should add multiple query parameters and return string directly', () => {
      const result = urlLiteral`/api/users`.queryString({ page: 1, limit: 10 });
      expect(result).toBe('/api/users?page=1&limit=10');
    });

    test('should handle empty object and return original path', () => {
      const result = urlLiteral`/api/users`.queryString({});
      expect(result).toBe('/api/users');
    });

    test('should handle null parameter and return original path', () => {
      const result = urlLiteral`/api/users`.queryString(null);
      expect(result).toBe('/api/users');
    });

    test('should handle undefined parameter and return original path', () => {
      const result = urlLiteral`/api/users`.queryString(undefined);
      expect(result).toBe('/api/users');
    });

    test('should ignore query parameters with null values', () => {
      const result = urlLiteral`/api/users`.queryString({ page: 1, search: null });
      expect(result).toBe('/api/users?page=1');
    });

    test('should ignore query parameters with undefined values', () => {
      const result = urlLiteral`/api/users`.queryString({ page: 1, search: undefined });
      expect(result).toBe('/api/users?page=1');
    });
    ...
    test('should handle string query parameters', () => {
      const result = urlLiteral`/api/users`.queryString({ name: 'john', role: 'admin' });
      expect(result).toBe('/api/users?name=john&role=admin');
    });

    test('should handle boolean query parameters', () => {
      const result = urlLiteral`/api/users`.queryString({ active: true, verified: false });
      expect(result).toBe('/api/users?active=true&verified=false');
    });

    test('should use & separator when path already contains ?', () => {
      const result = urlLiteral`/api/users?sort=name`.queryString({ page: 1 });
      expect(result).toBe('/api/users?sort=name&page=1');
    });

    test('should combine variables and query parameters', () => {
      const userId = 123;
      const result = urlLiteral`/api/users/${userId}/posts`.queryString({ page: 1, limit: 10 });
      expect(result).toBe('/api/users/123/posts?page=1&limit=10');
    });

    test('should handle query parameter values with special characters', () => {
      const result = urlLiteral`/api/search`.queryString({ q: 'hello world', filter: 'a=b' });
      expect(result).toContain('q=hello+world');
      expect(result).toContain('filter=a%3Db');
    });

    test('should handle query parameter with value 0', () => {
      const result = urlLiteral`/api/users`.queryString({ page: 0 });
      expect(result).toBe('/api/users?page=0');
    });

    test('should handle query parameter with empty string value', () => {
      const result = urlLiteral`/api/users`.queryString({ search: '' });
    ...
    test('should work with path that has params', () => {
      const result = urlLiteral`/api/users/:id`.param({ id: 123 }).queryString({ page: 1 });
      expect(result).toBe('/api/users/123?page=1');
    });
  });
});
